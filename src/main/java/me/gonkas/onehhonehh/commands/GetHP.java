...
public class GetHP implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (commandSender instanceof Player) {
            commandSender.sendMessage("§4[100HP 100H]§a Your HP is currently §4" + ((Player) commandSender).getHealth() + "§a.");
        } else if (args.length == 1) {

            Player target = Bukkit.getPlayer(args[0]);
            if (target != null) {
                commandSender.sendMessage("§4[100HP 100H]§c " + target + "§a's HP is currently §4" + target.getHealth() + "§a.");
            } else {
